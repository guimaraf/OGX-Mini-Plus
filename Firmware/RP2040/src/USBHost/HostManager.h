...
#define MAX_INTERFACES                                                         \
  MAX_GAMEPADS // This may change if support is added for audio or other
               // chatpads beside 360 wireless
...
public:
  ...
  inline bool setup_driver(const HostDriverType driver_type,
                           const uint8_t address, const uint8_t instance,
                           uint8_t const *report_desc = nullptr,
                           uint16_t desc_len = 0) {
    ...
    if (dev_idx == INVALID_IDX &&
        ((dev_idx = find_free_device_slot()) == INVALID_IDX)) {
      return false;
    }
    ...
    case HostDriverType::PS3:
      ...
      // descriptor length (176 bytes vs 148 bytes for original PS3 controller)
      if (desc_len == 176) {
        // Adapter: use HIDHost for generic HID gamepad handling
        interface.driver = std::make_unique<HIDHost>(gp_idx);
      } else {
        // Original PS3 controller: use native PS3Host driver
    ...
    case HostDriverType::XBOX360:
      ...
      break;
    case HostDriverType::XBOX360W: // Composite device, takes up all 4 gamepads
                                   // when mounted
    ...
    interface.driver->initialize(*interface.gamepad, device_slot.address,
                                 instance, report_desc, desc_len);

    return true;
  }

  inline void process_report(uint8_t address, uint8_t instance,
  ...
  inline void connect_cb(uint8_t address, uint8_t instance) {
    for (auto &device_slot : device_slots_) {
      if (device_slot.address == address &&
          device_slot.interfaces[instance].driver &&
          device_slot.interfaces[instance].gamepad) {
        device_slot.interfaces[instance].driver->connect_cb(
            *device_slot.interfaces[instance].gamepad, address, instance);
      }
    }
  }
  ...
  inline void disconnect_cb(uint8_t address, uint8_t instance) {
    for (auto &device_slot : device_slots_) {
      if (device_slot.address == address &&
          device_slot.interfaces[instance].driver &&
          device_slot.interfaces[instance].gamepad) {
        device_slot.interfaces[instance].driver->disconnect_cb(
            *device_slot.interfaces[instance].gamepad, address, instance);
      }
    }
  }
    ...
    for (auto &device_slot : device_slots_) {
      ...
      for (uint8_t i = 0; i < MAX_INTERFACES; ++i) {
        if (device_slot.interfaces[i].driver &&
            device_slot.interfaces[i].gamepad->new_pad_out()) {
          device_slot.interfaces[i].driver->send_feedback(
              *device_slot.interfaces[i].gamepad, device_slot.address, i);
          tuh_task();
        }
      }
    }
  ...
  void deinit_driver(DriverClass driver_class, uint8_t address,
                     uint8_t instance) {
    for (auto &device_slot : device_slots_) {
      if (device_slot.address == address) {
        device_slot.reset();
  ...
  inline uint8_t get_gamepad_idx(DriverClass driver_class, uint8_t address,
                                 uint8_t instance) {
    for (auto &device_slot : device_slots_) {
...
// #define MAX_INTERFACES MAX_GAMEPADS //This may change if support is added for
// audio or other chatpads beside 360 wireless
...
// 	inline bool setup_driver(const HostDriver::Type driver_type, const
// uint8_t address, const uint8_t instance, uint8_t const* report_desc =
// nullptr, uint16_t desc_len = 0)
// 	{
// 		uint8_t gp_idx = find_free_gamepad();
...
// 		if (hs_idx == INVALID_IDX) //This is a new device, else it's
// another interface on an already mounted device
// 		{
...
// 		HostSlot::Interface* interface =
// &host_slot->interfaces[instance];

// 		switch (driver_type)
// 		{
// 			// case HostDriver::Type::PS5:
// 			// 	interface->driver =
// std::make_unique<PS5Host>(gp_idx);
// 			// 	break;
// 			// case HostDriver::Type::PS4:
// 			// 	interface->driver =
// std::make_unique<PS4Host>(gp_idx);
// 			// 	break;
// 			// case HostDriver::Type::PS3:
// 			// 	interface->driver =
// std::make_unique<PS3Host>(gp_idx);
// 			// 	break;
// 			// case HostDriver::Type::DINPUT:
// 			// 	interface->driver =
// std::make_unique<DInputHost>(gp_idx);
// 			// 	break;
// 			// case HostDriver::Type::SWITCH:
// 			// 	interface->driver =
// std::make_unique<SwitchWiredHost>(gp_idx);
// 			// 	break;
// 			// case HostDriver::Type::SWITCH_PRO:
// 			// 	interface->driver =
// std::make_unique<SwitchProHost>(gp_idx);
// 			// 	break;
// 			// case HostDriver::Type::N64:
// 			// 	interface->driver =
// std::make_unique<N64Host>(gp_idx);
// 			// 	break;
// 			// case HostDriver::Type::PSCLASSIC:
// 			// 	interface->driver =
// std::make_unique<PSClassicHost>(gp_idx);
// 			// 	break;
// 			case HostDriver::Type::XBOXOG:
// 				interface->driver =
// std::make_unique<XboxOGHost>(gp_idx); 				break;
// case HostDriver::Type::XBOXONE: interface->driver =
// std::make_unique<XboxOneHost>(gp_idx); 				break;
// case HostDriver::Type::XBOX360: interface->driver =
// std::make_unique<Xbox360Host>(gp_idx); 				break;
// case HostDriver::Type::XBOX360W: //Composite device, takes up all 4 gamepads
// when mounted 				interface->driver =
// std::make_unique<Xbox360WHost>(gp_idx); 				break;
// 			default:
// 				// if (is_hid_gamepad(report_desc, desc_len))
// 				// {
// 				// 	interface->driver =
// std::make_unique<HIDHost>(gp_idx);
// 				// }
// 				// else
// 				{
// 					return false;
...
// 		interface->gamepad_idx = gp_idx;
// 		interface->gamepad = gamepads_[gp_idx];
// 		interface->driver->initialize(*interface->gamepad,
// host_slot->address, instance, report_desc, desc_len);
...
// 	inline void process_report(DriverClass driver_class, uint8_t address,
...
// 			{
// 				host_slot.interfaces[instance].driver->process_report(*host_slot.interfaces[instance].gamepad,
// address, instance, report, len);
// 			}
// 		}
// 	}
...
// host_slot.interfaces[i].gamepad->new_pad_out())
// 				{
// 					host_slot.interfaces[i].driver->send_feedback(*host_slot.interfaces[i].gamepad,
// host_slot.address, i); 					tuh_task();
// 				}
// 			}
// 		}
// 	}

//     void deinit_driver(DriverClass driver_class, uint8_t address, uint8_t
//     instance)
// 	{
// 		for (auto& host_slot : host_slots_)
// 		{
// 			if (host_slot.driver_class == driver_class &&
// host_slot.address == address)
// 			{
...
// 			for (size_t i = 0; i < map.num_ids; i++)
// 			{
// 				if (ids.pid == map.ids[i].pid && ids.vid ==
// map.ids[i].vid)
// 				{
// 					return map.type;
// 				}
...
// 	static inline HostDriver::Type get_type(const tuh_xinput::DevType
// xinput_type)
// 	{
// 		switch (xinput_type)
// 		{
...
// 	inline uint8_t get_gamepad_idx(DriverClass driver_class, uint8_t
// address, uint8_t instance)
// 	{
// 		for (auto& host_slot : host_slots_)
// 		{
// 			if (host_slot.driver_class == driver_class &&
// host_slot.address == address && instance < MAX_INTERFACES)
// 			{
// 				return
// host_slot.interfaces[instance].gamepad_idx;
// 			}
// 		}
// 		return INVALID_IDX;
// 	}
...
// 	bool is_hid_gamepad(const uint8_t* report_desc, uint16_t desc_len)
// 	{
// 		std::array<uint8_t, 6> start_bytes = { 0x05, 0x01, 0x09, 0x05,
// 0xA1, 0x01 }; 		if (desc_len < start_bytes.size())
// 		{
// 			return false;
// 		}
// 		for (size_t i = 0; i < start_bytes.size(); ++i)
