...
namespace PS3 {
static constexpr uint8_t MAGIC_BYTES[8] = {0x21, 0x26, 0x01, 0x07,
                                           0x00, 0x00, 0x00, 0x00};
...
const uint8_t DEFAULT_OUT_REPORT[] = {
    0x01, 0xff, 0x00, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x27,
    0x10, 0x00, 0x32, 0xff, 0x27, 0x10, 0x00, 0x32, 0xff, 0x27, 0x10, 0x00,
    0x32, 0xff, 0x27, 0x10, 0x00, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00};
...
struct OutReport {
  ...
  struct Rumble {
    ...
    uint8_t right_motor_on; /* Right (small) motor on/off, only supports values
                               of 0 or 1 (off/on) */
    uint8_t left_duration;  /* Left motor duration (0xff means forever) */
    uint8_t left_motor_force; /* left (large) motor, supports force values from
                                 0 to 255 */
  } rumble;
  uint8_t reserved1[4];
  uint8_t
      leds_bitmap; /* bitmap of enabled LEDs: LED_1 = 0x02, LED_2 = 0x04, ... */
  struct LEDs {
    uint8_t time_enabled; /* the total time the led is active (0xff means
                             forever) */
    uint8_t duty_length;  /* how long a cycle is in deciseconds (0 means "really
                             fast") */
    uint8_t enabled;
...
static constexpr uint8_t DEFAULT_BT_INFO_HEADER[] = {
    0xFF, 0xFF, 0x00, 0x20, 0x40, 0xCE, 0x00, 0x00, 0x00,
...
struct BTInfo {
  ...
  BTInfo() {
    std::memcpy(device_address, DEFAULT_BT_INFO_HEADER,
                sizeof(DEFAULT_BT_INFO_HEADER));

    std::mt19937 gen(12345);
    std::uniform_int_distribution<uint16_t> dist(0, 0xFF);

    for (uint8_t i = 4; i < sizeof(device_address); i++) {
      device_address[i] = static_cast<uint8_t>(dist(gen));
    }
...
static const uint8_t REPORT_DESCRIPTORS[] = {
    ...
    0x04, // Usage (Joystick)
    0xA1,
    0x01, // Collection (Physical)
    0xA1,
    0x02, //   Collection (Application)
    0x85,
    0x01, //     Report ID (1)
    0x75,
    0x08, //     Report Size (8)
    ...
    0x03, //     Input (Const,Var,Abs,No Wrap,Linear,Preferred State,No Null
          //     Position) NOTE: reserved byte
    0x75,
    0x01, //     Report Size (1)
    0x95,
    0x13, //     Report Count (19)
    0x15,
    0x00, //     Logical Minimum (0)
    0x25,
    ...
    0x81,
    0x03, //     Input (Const,Var,Abs,No Wrap,Linear,Preferred State,No Null
          //     Position) NOTE: 32 bit integer, where 0:18 are buttons and
          //     19:31 are reserved
    0x15,
    0x00, //     Logical Minimum (0)
    0x26,
    ...
    0x01, //     Usage (Pointer)
    0xB1,
    0x02, //     Feature (Data,Var,Abs,No Wrap,Linear,Preferred State,No Null
          //     Position,Non-volatile)
    0xC0, //   End Collection
    0xA1,
    0x02, //   Collection (Application)
...
static constexpr uint8_t OUTPUT_0x01[] = {
    0x01, 0x04, 0x00, 0x0b, 0x0c, 0x01, 0x02, 0x18, 0x18, 0x18, 0x18,
    0x09, 0x0a, 0x10, 0x11, 0x12, 0x13, 0x00, 0x00, 0x00, 0x00, 0x04,
    0x00, 0x02, 0x02, 0x02, 0x02, 0x00, 0x00, 0x00, 0x04, 0x04, 0x04,
    0x04, 0x00, 0x00, 0x04, 0x00, 0x01, 0x02, 0x07, 0x00, 0x17, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// calibration data
static constexpr uint8_t OUTPUT_0xEF[] = {
    0xef, 0x04, 0x00, 0x0b, 0x03, 0x01, 0xa0, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x01, 0xff, 0x01, 0xff, 0x01, 0xff, 0x01, 0xff,
...
static constexpr uint8_t OUTPUT_0xF7[] = {
    0x02, 0x01, 0xf8, 0x02, 0xe2, 0x01, 0x05, 0xff, 0x04, 0x03, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
